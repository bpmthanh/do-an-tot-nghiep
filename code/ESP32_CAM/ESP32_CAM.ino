...
//輸入WIFI連線帳號密碼
const char* ssid     = "ThanhBui";   //your network SSID
const char* password = "autoship";   //your network password

//輸入AP端連線帳號密碼
const char* apssid = "ESP32-CAM";
const char* appassword = "12345678";         //AP password require at least 8 characters.

String myScript = "/macros/s/AKfycbz59qe2EoLVZLZ_jZF_hBRFIpo_e3qtt1s1EG2Q78htc05QHRdZlLjFA7byo6EPIQl8Ww/exec";    //Create your Google Apps Script and replace the "myScript" path.
...
String myWarningFoldername = "&myFoldername=WARNING";
...
<html>
    ...
    <body>
      ...
      <tr><td><button id="restart">Restart</button></td><td><button id="get-still">Stream</button></td><td><button id="stop">Stop</button></td></tr>
...
void ExecuteCommand()
{
  ...
  if (Serial.available()) {
    String receivedData = Serial.readString();  // Đọc dữ liệu nhận được
    Serial.print("Received data: ");
    Serial.println(receivedData);
    if (receivedData.startsWith("OPEN")) {
      Feedback=SendCapturedImage();
    if (Feedback=="") Feedback="The image failed to send. <br>The framesize may be too large.";
    }
    if (receivedData.startsWith("WARNING")) {
      Feedback=SendWarningCapturedImage();
    if (Feedback=="") Feedback="The image failed to send. <br>The framesize may be too large.";
    }
  }
...
String SendWarningCapturedImage() {
  const char* myDomain = "script.google.com";
  String getAll="", getBody = "";
  ...
  if (client_tcp.connect(myDomain, 443)) {
    ...
    String Data = myLineNotifyToken+myWarningFoldername+myFilename+myImage;
